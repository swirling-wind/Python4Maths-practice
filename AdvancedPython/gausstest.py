...
def speed(method,maxSize=400):
    ...
    while n <= maxSize:
        ...
        print("%4d %10.4f ± %.2f sec" % (n,mean(t),stdev(t)),"(x %.2f)"%(mean(t)/prev) if prev > 0 else "")
        prev = mean(t)
        n *= 2
    gc.enable()
...
def parspeed(method,maxSize=400):
    ...
    while n <= maxSize:
        ...
        T = timeit.repeat(stmt="method(A,b)",setup=f"A,b=randP({n})",
                      timer=lambda : np.array([time.perf_counter(),time.process_time()]),repeat=5,number=1,globals=locals())
        #CPU += time.process_time()
        CPU = [ x[1] for x in T]
        t = wall = [x[0] for x in T]
        print("%4d elapsed %10.4f ± %.2f sec" % (
            n,mean(t),stdev(t)),"(x %.2f)"%(mean(t)/prev) if prev > 0 else "",
             "%.2f threads used on average"%(sum(CPU)/sum(t)))
        prev = mean(t)
        n *= 2
    gc.enable()
